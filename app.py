...
# def hi():
#     name = 'kang kyung mi'
#     return render_template('hello.html',name=name) # html에서 쓰는 변수명=파이썬 함수에서 정의된 변수명

# @app.route('/board')
# def board_list():
#     db = pymysql.connect(host='database-1.c1sfhfewzr4b.ap-northeast-2.rds.amazonaws.com',db='pbldb',port=3306,passwd=db_password,user='admin')
...
def write():
    if request.method == 'POST':
        name = request.form.get('name',False) #innput 상자의 네임
        ...
        if error is None:
            db = pymysql.connect(host='database-1.c1sfhfewzr4b.ap-northeast-2.rds.amazonaws.com',port=3306,db='pbldb',passwd=db_password,user='admin')
            cursor = db.cursor()
            cursor.execute(
            ...
            if file :
                ...
                cursor.execute(
                    ...
                    (board_id, file.filename,  's3-myboard305files', 'images/' + file.filename,  file.content_type)
                )
            db.commit()
            cursor.close()
            db.close()
...
def download():
    ...
    if result :
        ...
        print("-------------------------------------------------------------------")
        return send_file(f'firstweb/1.jpg', mimetype='image/jpg') # 확장자 통일시키기
